        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-200/60 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none">
          ...
          {success ? (
            ...
            <form onSubmit={handleSignup} className="space-y-6">
              <div>
                <label htmlFor="clinicName" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1 cursor-pointer">Clinic Name</label>
                <div className="relative">
                  <Stethoscope className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} aria-hidden="true" />
                  <input 
                    id="clinicName"
                    name="clinicName"
                    type="text" 
                    ...
                    placeholder="City Care Clinic… e.g. City Care Clinic"
              ...
              >
                {loading ? <Loader2 className="animate-spin" size={20} aria-hidden="true" /> : <>Create Account <ArrowRight size={20} aria-hidden="true" /></>}
              </button>
            </form>
          )}
        </div>
